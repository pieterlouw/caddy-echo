// Package tcpechomain contains the functions for starting tcpEcho
package tcpechomain
...
import (
	...
	// Plug in TCPEcho
	_ "github.com/pieterlouw/caddy-tcpecho/tcpecho"
)

func init() {
	caddy.TrapSignals()
	caddy.DefaultConfigFile = "Echofile"
	setVersion()

	flag.StringVar(&conf, "conf", "", "Echofile to load (default \""+caddy.DefaultConfigFile+"\")")
...
func Run() {
	...
	caddy.AppName = echoName
	caddy.AppVersion = echoVersion
	...
	// Get Echofile input
	echofile, err := caddy.LoadCaddyfile(serverType)
	if err != nil {
		mustLogFatal(err)
	}

	// Start your engines
	instance, err := caddy.Start(echofile)
	if err != nil {
		mustLogFatal(err)
	}
...
func confLoader(serverType string) (caddy.Input, error) {
	...
	if conf == "stdin" {
		return caddy.CaddyfileFromPipe(os.Stdin, "tcpecho")
	}
